...
## 双仓库同步流程（私人 main ↔ 开源 shuatibao）

本项目同时维护两个仓库：
- `origin` → 私人仓库 `learn-or-die-lite`, 主分支 `main`, 日常开发
- `shuatibao` → 公开仓库 `shuatibao`, 主分支基于本地 `opensource` 分支

### 分支职责

- **main**: 私人开发主线。含 VPS IP / 测试账号 / 本 DEV_NOTES / PPT 等私人内容。push 到 `origin`。
- **opensource**: 开源版。跟 main 内容大部分一致，但删 PPT、IP 替换占位符、加 README/LICENSE/CONTRIBUTING。push 到 `shuatibao` 的 `main`。

### 日常工作流（开发后同步到开源版）

```bash
# 1. 平时在 main 上开发, push 私人仓库
git checkout main
# ... 改代码 ...
git commit -m "..."
...
# git 智能处理: rename (CLAUDE.md→DEV_NOTES.md) / delete (PPT) 都会保留 opensource 状态

# 3. push 前强制扫一遍敏感信息
git grep -in "82\.156\.139\.33\|lornax@local\|123123\|sk-sp-04\|zp1-1428145534"
# 0 行才安全. 有匹配就改成占位符 + git add + git commit --amend

# 4. push 到 shuatibao 的 main
git push shuatibao opensource:main
```
...
1. **立刻撤销 secret 本身**（重发 key / 改密码 / 关 endpoint）— 这是最重要的
2. 私人仓库可暂不动 history
3. 开源仓库 history 重写需要 `git filter-repo` + force-push（GitHub cache 仍可能保留, 撤 key 是终极防线）
